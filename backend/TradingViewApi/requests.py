...
class TradingViewApi:
    ...
    async def get_indicators_data(self, range: int, timeframe: str, currency: str):
        ...
        for k, v in self.indicatorIds_dict.items():
            response = self.send_req(self.sessionId, self.sessionSign, range, timeframe, currency, v)
            task = asyncio.Task(response, loop=asyncio.get_event_loop(), name=k)
            background_tasks.append(task)

        await asyncio.gather(*background_tasks, return_exceptions=True)
        for i in background_tasks:
            try:
                res[i.get_name()] = i.result()[0:10]
