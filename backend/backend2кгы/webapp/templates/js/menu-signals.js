...
const menu_frame   = document.querySelector("#menu-frame");
...
function window_resize_handler(event) {
    const frame_style = getComputedStyle(menu_frame, null);
    let padding = frame_style.getPropertyValue('padding-left');
    padding = parseInt(padding.replace('px', ''));
    btn_main.style.width = (menu_frame.offsetWidth - 2 * padding) + "px";
}
