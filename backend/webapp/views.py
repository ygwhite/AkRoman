...
from settings import BACKEND_LOGGER as log, WSS_URL, WSS_HTTPS_URL, Fear_And_Greed_API_URL
...
def menu(request, *args, **kwargs):
    ...
    if request.method == "GET":
        ...
        # получаем выбранные юзером раннее таймфремы из таблицы
        timeframes_selected_instance_all = UserTimeframes.objects.filter(user__uid=tg_uid).all()
        currency_pairs_selected_tf = {}
        if len(timeframes_selected_instance_all) > 0:
            timeframes_selected_instance = timeframes_selected_instance_all[0]
            selected_timeframes = json.loads(timeframes_selected_instance.timeframe)

            # создаем словарь для более быстрого поиска по валютами

            for cp in selected_timeframes:
                currency_pairs_selected_tf[(cp[0], cp[1])] = cp[2]
        context["currency_pairs_selected_tf"] = currency_pairs_selected_tf
        if len(context["timeframes"]) > 0:
            context["initial_tf"] = context["timeframes"][0]
        context["currency_pairs_selected_tf"] = currency_pairs_selected_tf
        all_currencypairs = CurrencyPair.objects.all()
        for cur in all_currencypairs:
            cur.tf_selecter = currency_pairs_selected_tf.get((cur.first.abbr, cur.last.abbr), [])
